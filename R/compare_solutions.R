...
compare_solutions <- function(attempt_file, solution_file, ...) {
  ...
  solution_list <- source(system.file("testing", solution_file, package = "teachr"), local = TRUE)
  attempt_list <- source(system.file("testing", attempt_file, package = "teachr"), local = TRUE)
